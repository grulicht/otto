...
choose_profile() {
    ...
    echo "  1) ${GREEN}beginner${NC}    - Explains everything, always asks before acting"
    echo "  2) ${BLUE}balanced${NC}    - Read auto, writes confirm, destructive deny (recommended)"
    echo "  3) ${YELLOW}autonomous${NC}  - Maximum automation, minimal confirmations"
    echo "  4) ${RED}paranoid${NC}    - Everything requires explicit approval"
    echo ""
